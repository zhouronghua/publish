...
这里的控制流图比我们常规见到的控制流图要复杂一点，因为这是基于LLVM IR生成的控制流图，类似于汇编语言，中间还涉及一些从内存加载到寄存器（LLVM IR里面的寄存器一般称为虚拟寄存器，最终发射对应硬件的指令的时候，需要映射到实际的物理寄存器，如果物理寄存器不够的，需要做溢出处理）的操作。
...
我这里用的是llvm11的clang，生成出来的llvm ir的module的原始attributes是35行，但这个attributes（optnone会抑制优化）无法生成DAG，需要改成36行的attributes。
...
如果是命令行连接的linux系统，由于没有window可以展示，会提示生成了dot文件。然后大家可以选择将dot文件转换成svg或者png等图片格式，我更喜欢svg，因为里面的字符串还是字符串形式的存在，不像png完全是图片，不方便拷贝。但svg的图有时非常大，在文档里面当做截图的话容易变形。生成svg的命令如下，假定提示生成的dot文件名为dag._Z8dag_testiii-3c598e.dot，如果要生成png，只需要把参数里面的-Tsvg换成-Tpng，生成的文件后缀名也改成png就可以：
...
直接对上面生成的bc文件跑opt，发现优化之后的llvm ir和优化前的llvm ir几乎完全一样，没有达到预期的优化效果。最后删掉属性里面的optnone之后终于能正常优化了。
...
先秀一下没做CSE之前的LLVM IR（省去不关心的各种attributes，但自己跑的时候注意把optnone的attribute删除掉再跑）：
...
promoteMemoryToRegister有3个入参。F表示一个函数。DT的类型是DominatorTree，支配树的概念要在第7章SSA里面才会讲，感兴趣的可以先看，不过不知道也关系不大，可以先简单理解控制流图上节点的先后关系。AC的类型是AssumptionCache，直译就是假定缓存，意思是把变量v1先假定分配给寄存器r1，后面如果整体推导下来没什么问题就这样定了，如果有问题再改这个假定。
...
下面详细说明一下INITIALIZE_PASS_BEGIN、INITIALIZE_PASS_DEPENDENCY和INITIALIZE_PASS_END的定义。相关定义在llvm\include\llvm\PassSupport.h中。
...
LLVM的early-cse的实现，主要体现在llvm\lib\Transforms\Scalar\EarlyCSE.cpp里面，下面解析一下该文件里面的实现，方便大家深入理解CSE的具体实现。
...
![图](images/image_0020.tiff)

等价于

(! P)? B: A

![图](images/image_0021.tiff)
...
但实际上LLVM自己的命名规范里面还是倾向于使用驼峰等命名规则的，也希望变量名用小写，不过实际开发过程中并没有很好的落实[2]。总的来说，除了部分大家都很清楚的简称外，变量名直接用类型的单次首字母做简称的做法，虽然对写代码的人减少了敲的时间，但读代码的人肯定是个挑战。AllocatorTy是个别名，是RecyclingAllocator这个内存申请类的简称。ScopedHTType也是ScopedHashTable的简称，主要记录变量的作用域，是一个变量到它作用域的哈希表映射类型。
...
函数handleBranchCondition进行分支相关处理，如果分支的处理会影响变量的代，则返回true，否则返回false。这里涉及到了worklist算法，更详细的算法思想后面会有更细节的讨论，这里主要关注把工作列表算法当做把需要分析的数据不断放到处理队列，然后时间处理过程中就是反复遍历工作队列，直到工作队列为空的计算过程。
