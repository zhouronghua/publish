...
本章是系列文章的案例学习，不属于正篇，主要介绍了TensorFlow引入的XLA的优化算法，XLA的优化主要是合并，即通过多个HLO的合并减少跨HLO的内存拷贝过程。XLA也有很多局限性，XLA没有数据切分的功能，但有时候如果参数特别多的场景下，分割是必须的，这是就需要额外做处理。当前最主流的编译器领域的编译优化工具还是mlir（llvm下的一个子项目）。
...
![图](images/image_0181.png)

A.4.1 定义所有指令的所有逻辑缓冲区
...
![图](images/image_0182.png)

A.5 buffer指派的功能

A.5.1 将能够重用的buffer尽可能重用
